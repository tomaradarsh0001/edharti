...
// 				        DB::raw('SUM(demand_details.paid_amount) as total_paid_amount')
// 				    )
// 				    ->when(!is_null($startDate), function ($q) use ($startDate) {
// 				        return $q->where('demand_details.updated_at', '>=', Carbon::createFromFormat('d-m-Y', $startDate)->startOfDay());
// 				    })
// 				    ->when(!is_null($endDate), function ($q) use ($endDate) {
// 				        return $q->where('demand_details.updated_at', '<=', Carbon::createFromFormat('d-m-Y', $endDate)->endOfDay());
// 				    })
// 				    ->groupBy('demand_details.subhead_id', 'i.item_name')
// 				    ->get();
			
// 			$processingfee = Payment::select('status', DB::raw('SUM(amount) as total_amount'))
// 		    						 ->where('type', 1550)
// 		    						->where('status', 1546)
// 		    						->when(!is_null($startDate), function ($q) use ($startDate) {
...
// 				    })
// 				    ->when(!empty($filterService), function ($q) use ($filterService) {
// 				        $q->where('payments.status', '=', $filterService);
// 				    })
// 				    ->get();
        ...
        if (curl_errno($ch)) {
            ...
            if (count($apiResponse) > 1) {
                ...
                if (strtoupper($orderStatus) == "SUCCESS") {
                    ...
                    dd($data, $curl_output, $apiResponse);
                }
            } else {
