...
class ApplicationController extends Controller
{
            ...
            case 'luc':
                ...
                $query2 = DB::table('land_use_change_applications as lca')
                    ...
                    ->select(
                        ...
                        'app.is_objected',
                        'property_masters.created_at as latest_moved_at',
                        DB::raw('NULL as flat_id'), // Add NULL for flat_id on dated 07/01/25 By Lalit Tiwari
                        DB::raw('NULL as flat_number'), // Add NULL for flat_number on dated 07/01/25 By Lalit Tiwari
        ...
        if ($applicationDetails) {
            ...
            if ($isOfficeViewTheApplication) {
                ...
                if (!empty($property['id'])) {
                    ...
                    //         ->get('https://ldo.gov.in/eDhartiAPI/Api/GetValues/PropertyDocList?PropertyID=' . $property['old_propert_id']);
                    ...
                    //                 $scannedFiles['files'][] = $value['PropertyFileNameDcrpt'];
                    //             }
                    //         } else {
    ...
    public function applicationAction(Request $request)
    {
        return DB::transaction(function () use ($request) {
            ...
            if ($action == 'PROOFREADINGLINK') {
                ...
                // Check for conversion application demand and subhead before letter generation
                if ($serviceType == 'CONVERSION') {
                    $model = '\\App\\Models\\' . $application->model_name;
                    $applicationDetails = $model::where('id', $application->model_id)->first();

                    if (!$applicationDetails) {
                        $response = ['status' => false, 'message' => 'Application details not found.'];
                        return response()->json($response);
                    }
                    ...
                    $demand = Demand::where('old_property_id', $oldPropertyId)
                        ->where(function ($query) use ($applicationDetails, $splittedColumn) {
                            if (!empty($applicationDetails->$splittedColumn)) {
                                $query->where($splittedColumn, $applicationDetails->$splittedColumn);
                            } else {
                                $query->whereNull($splittedColumn);
                            }
                        })
                        ->whereIn('status', [getServiceType('DEM_PENDING'), getServiceType('DEM_PARTIAL_PAID'), getServiceType('DEM_DRAFT')])
                        ->first();
            ...
            /** action for object  by Nitin 13dec2024*/
                ...
                /** end code to check pending demand */
    ...
    public function generateLetter($application, $latestAppAction, $action, $request)
    {
        ...
        // dd(getServiceNameById($application->service_type));
         if ($application->service_type == getServiceType('SUB_MUT')) {
            ...
            // $latestLesseeDetails = PropertyTransferredLesseeDetail::where('property_master_id', $propertyDetails->id)
            //     ->where(function ($query) use ($childId) {
            //         if (is_null($childId)) {
            //             $query->whereNull('splited_property_detail_id');
            //         } else {
            //             $queryresult = $query->where('splited_property_detail_id', $childId);
            //             if(!$queryresult){ //if it has no records
            //                 //than get latest property_master_id batch id records

            //             }

            //         }
            //     })
            //     ->where('batch_transfer_id', function ($q) use ($propertyDetails, $childId) {
            //         $q->selectRaw('MAX(batch_transfer_id)')
            //             ->from('property_transferred_lessee_details')
            //             ->where('property_master_id', $propertyDetails->id)
            //             ->where(function ($sub) use ($childId) {
            //                 if (is_null($childId)) {
    ...
    //For Conversion Application Final By - Lalit (01-01-2026) 
    /* public function finalizeConversionApplication($conversionApplication)
    {
        ...
        if ($user && $propertyMasterId) {
            ...
            if (!empty($masterProperty)) {
                ...
                //Fetch max batch_transfer_id from PropertyTransferredLesseeDetail Table
                $isPrevBatchId = PropertyTransferredLesseeDetail::where('property_master_id', $masterProperty->id)->max('batch_transfer_id');
                if ($isPrevBatchId) {
                ...
                PropertyTransferredLesseeDetail::create([
                    ...
                    'lessee_age' => $user->applicantUserDetails->age,
                    'lessee_pan_no' => $user->mobile_no,
                    'lessee_aadhar_no' => $user->mobile_no,
                ...
                $coApplicants = CoApplicant::where('model_name', 'ConversionApplication')->where('model_id', $conversionApplication->id)->get();
                if ($coApplicants) {
                ...
                // Fetch Details from Property Transferred Lessee Detail Table for Co-Applicant and insert in Current Lessee Details Table
                $propertyTransferDetails = PropertyTransferredLesseeDetail::where('property_master_id', $masterProperty->id)
                    ->where('batch_transfer_id', $batch_transfer_id)
                    ->get();

                $leaseNames = []; // Initialize an array to store all lessee names

                if ($propertyTransferDetails->isNotEmpty()) {
                    foreach ($propertyTransferDetails as $propertyTransferDetail) {
                        // Add each lessee name to the array
                    ...
                    $leaseNameString = ''; // Or null, or whatever default you want
                }
