        ...
        <div class="col-lg-12 mb-4">
            <div class="card widget-card">
                <div class="card-body">
                    ...
                    @php
						     $processingTotal = collect($processingfee)->sum('total_amount');
						    $headwiseTotal = collect($headwise)->sum('total_paid_amount');
						    $grandTotal = $processingTotal + $headwiseTotal;
					@endphp                     
					<table class="table table-bordered">
					    ...
					    <tbody>	
					        @foreach ($processingfee as $index=>$item)            
					            <tr>
					                <td id="submitted_service" class="submitted-data"> {{ getServiceNameById($item->type) =='Application' ? "Application Processing Fee" : getServiceNameById($item->type)  }}</td>
					                <td  class="submitted-data">
					                 <a href="javascript:;" class="app-query-link" data-status="processingfee" data-service="{{$item->type}}"  > 
					                ₹ {{ customNumFormat(round($item->total_amount ?? 0, 2)) }}
					                </a>
					                </td>					                          
					            </tr>
					        @endforeach				   
				       		     
					       @foreach ($headwise as $index=>$item)            
					            <tr>
					                <td id="submitted_service" class="submitted-data"> {{ $item->subhead_name }}</td>
					                <td  class="submitted-data">
					                 <a href="javascript:;" class="app-query-link" data-status="demandpaid" data-service="{{$item->subhead_id}}"  > 
					                ₹ {{ customNumFormat(round($item->total_paid_amount ?? 0, 2)) }}
					                </a>
					                </td>					                          
					            </tr>
					        @endforeach       
					    ...
					    <tfoot>
						    <tr class="table-secondary">
						        ...
						        <th class="text-wrap" style="max-width: 200px; white-space: normal;">₹ {{ customNumFormat(round($grandTotal, 2)) }}          						 </th>        
						    </tr>
						</tfoot>
